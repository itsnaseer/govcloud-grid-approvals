...
def send_approval_decision_to_salesforce(record_id, decision, user_email):
    ...
    def refresh_salesforce_token():
        ...
        response = requests.post(
            os.getenv("SALESFORCE_TOKEN_URL"),  # Ensure this is set in .env
            data={
                "grant_type": "password",
        ...
        if response.status_code == 200:
            ...
            os.environ["SALESFORCE_ACCESS_TOKEN"] = new_token  # Update in-memory
            logger.info("Salesforce token refreshed successfully!")
            return new_token
        else:
            logger.error(f"Failed to refresh Salesforce token: {response.text}")
            return None
    ...
    if response.status_code == 401 and "INVALID_SESSION_ID" in response.text:
        logger.warning("Salesforce session expired. Attempting to refresh token...")
